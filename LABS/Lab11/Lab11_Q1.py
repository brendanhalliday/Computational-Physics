...
if __name__ == "__main__":
    ...
    for t in T:
        ...
        for k in range(steps):
            ...
            eplot.append(E)
        TOTAL_ENERGY.append(eplot[-1])
        # Make the graph
        plt.plot(eplot)
        plt.ylabel("Energy")
        plt.xlabel("Step Number")
        plt.title(r"Energy Plot for $k_{B}T = $" + str(int(t)))
        plt.savefig("Lab11Q1plot1_" + str(int(t)) + ".png", dpi = 300, bbox_inches = "tight")
        ...
        average_n = np.sum(energy_frequency*n_vals)/np.sum(energy_frequency)
        AVERAGE_N.append(average_n)
        ...
        plt.title(r"Energy Frequency Histogram for $k_{B}T = $" + str(int(t)))
    ...
    plt.title(r"Total energy at equilibrium vs. $k_{B}T$")
    plt.savefig("Energytemp.png", dpi = 300, bbox_inches = "tight")

    plt.figure()
    plt.plot(T, AVERAGE_N, ".")
    plt.xlabel(r"$k_{B}T$")
    plt.ylabel("Average n")
    plt.title(r"Average n as a function of $k_{B}T$")
    plt.savefig("ntemp.png", dpi = 300, bbox_inches = "tight")
